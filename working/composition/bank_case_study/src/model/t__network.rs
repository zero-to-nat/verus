...
impl Network {
    ...
    pub open spec fn next(pre: Self, post: Self, host_msg_ops: MessageOps, other_sent_msgs: Set<Message<Seq<u8>>>) -> bool
    {
    ...
    pub proof fn next_inv(pre: Self, post: Self, host_msg_ops: MessageOps, other_msgs: Set<Message<Seq<u8>>>)
        requires
            Self::next(pre, post, host_msg_ops, other_msgs),
            Self::inv(pre)
        ensures
            Self::inv(post)
    {}
