...
pub trait ServiceConstants : NetworkedStateMachineConstants {
}

pub trait ServiceDefinition<C: ServiceConstants> : NetworkedStateMachine<C> {
    type ServiceRequest;
    type ServiceReply;
    ...
    spec fn is_service_request(c: C, m: Message<Seq<u8>>, msgs: Set<Message<Seq<u8>>>) -> bool
        ;

    spec fn is_service_reply(c: C, m: Message<Seq<u8>>, msgs: Set<Message<Seq<u8>>>) -> bool
        ;
...
pub struct AbstractService<C: ServiceConstants, S: ServiceDefinition<C>> {
    pub p0: PhantomData<C>,
    pub p1: PhantomData<S>
}

impl<C: ServiceConstants, S: ServiceDefinition<C>> AbstractService<C, S> {
    ...
    pub open spec fn next(pre: S, post: S, msg_ops: MessageOps) -> bool
    {
        ...
        &&& (forall |m| #[trigger] S::is_service_request(pre.constants(), m, msg_ops.recv) ==> 
        {
            let request = m.replace_msg(S::parse_request_spec(m.msg).unwrap());
            &&& post.requests().contains(request)
        })
        &&& (forall |m| #[trigger] S::is_service_reply(pre.constants(), m, msg_ops.send)  ==> 
        {
        ...
        &&& (forall |req| #[trigger] post.requests().contains(req) ==> {
            ...
            ||| (exists |m: Message<Seq<u8>>| S::is_service_request(pre.constants(), m, msg_ops.recv) && req == #[trigger] m.replace_msg(S::parse_request_spec(m.msg).unwrap()))
        })
        &&& (forall |repl| #[trigger] post.replies().contains(repl) ==> {
            ||| pre.replies().contains(repl)
            ||| (exists |m: Message<Seq<u8>>| S::is_service_reply(pre.constants(), m, msg_ops.send) && repl == #[trigger] m.replace_msg(S::parse_reply_spec(m.msg).unwrap()))
        })
        &&& pre.requests().subset_of(post.requests())
        &&& pre.replies().subset_of(post.replies())
    }

    pub open spec fn is_service_request(c: C, m: Message<Seq<u8>>, msgs: Set<Message<Seq<u8>>>) -> bool {
        &&& msgs.contains(m) 
    ...
    pub open spec fn is_service_reply(c: C, m: Message<Seq<u8>>, msgs: Set<Message<Seq<u8>>>) -> bool {
        &&& msgs.contains(m) 
    ...
    pub open spec fn stutter(pre: S, msg_ops: MessageOps) -> bool {
        &&& forall |m| #[trigger] msg_ops.recv.contains(m) ==> !S::is_service_request(pre.constants(), m, msg_ops.recv)
        &&& forall |m| #[trigger] msg_ops.send.contains(m) ==> !S::is_service_reply(pre.constants(), m, msg_ops.send)
    }
}

pub trait Service<C: ServiceConstants> : ServiceDefinition<C> + StateMachine<C, MessageOps> {
    proof fn service_request_abs(s: Self, m: Message<Seq<u8>>, msgs: Set<Message<Seq<u8>>>)
        requires
            Self::is_service_request(s.constants(), m, msgs)
        ensures
            AbstractService::<C, Self>::is_service_request(s.constants(), m, msgs),
        ;

    proof fn service_reply_abs(s: Self, m: Message<Seq<u8>>, msgs: Set<Message<Seq<u8>>>)
        requires
            Self::is_service_reply(s.constants(), m, msgs)
        ensures
            AbstractService::<C, Self>::is_service_reply(s.constants(), m, msgs),
        ;
    ...
    proof fn stutter_abs(pre: Self, msg_ops: MessageOps)
        ensures Self::stutter(pre, msg_ops) <==> AbstractService::<C, Self>::stutter(pre, msg_ops)
        ;
}
}
