...
module ClientDistributedSystem refines RefinementTheorem {
    ...
    ghost predicate ValidStep(c: Constants, v: Variables, v': Variables, evt: Option<Host.Spec.Event>)
    {
        exists msgOps :: NextStep(c, v, v', evt, HostActionStep(0, msgOps))
    }

    ghost predicate NoOp(c: Constants, v: Variables, v': Variables)
    {
        exists msgOps :: NextStep(c, v, v', None, HostActionStep(0, msgOps))
    }
    ...
    ghost predicate NonTriviality_Part1Spec(c: Constants, behavior: seq<Variables>)
    {
        ...
        && 0 < |behavior|
        && Init(c, behavior[0])
        && (forall i:nat | i < |behavior|-1 :: 
            (|| ValidStep(c, behavior[i], behavior[i+1], None) 
            || ValidStep(c, behavior[i], behavior[i+1], Some(Network.Host.Spec.SendRequest)) 
            || ValidStep(c, behavior[i], behavior[i+1], Some(Network.Host.Spec.ReceiveResponse))))
    ...
    ghost predicate NonTriviality_Part2Spec(c: Constants, part1: seq<Variables>, part2: seq<Variables>)
    {
        ...
        && part2[0..|part1|] == part1
        && 0 < |part2[|part1|..]|
        && (forall i:nat | |part1| <= i < |part2| - 1 :: NoOp(c, part2[i], part2[i+1]))
    ...
    ghost predicate NonTriviality_Part3Spec(c: Constants, part1: seq<Variables>, part2: seq<Variables>, part3: seq<Variables>)
    {
        ...
        && part3[0..|part2|] == part2
        && 0 < |part3[|part2|..]|
        && (forall i:nat | |part2| <= i < |part3|-1 :: 
            (|| ValidStep(c, part3[i], part3[i+1], None) 
            || ValidStep(c, part3[i], part3[i+1], Some(Network.Host.Spec.SendRequest)) 
            || ValidStep(c, part3[i], part3[i+1], Some(Network.Host.Spec.ReceiveResponse))))
        && part3[|part3|-1].WF(c)
        ...
        ensures NonTriviality_Part3Spec(c, part1, part2, behavior)
    {
        ...
        assert Host.ReceiveResponse(c.hosts[0], behavior[4].hosts[0], behavior[5].hosts[0], Some(Host.Spec.ReceiveResponse), msgOps);
        assert NextStep(c, behavior[4], behavior[5], Some(Host.Spec.ReceiveResponse), HostActionStep(0, msgOps));
    }
}