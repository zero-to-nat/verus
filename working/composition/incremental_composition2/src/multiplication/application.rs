...
/*
impl ApplicationImpl<InductiveMultiplicationApplicationSpec> for InductiveMultiplicationApplication {
    type Constants = (SocketConnection, SocketConnection);
    
    open spec fn abs(s: Self) -> InductiveMultiplicationApplicationSpec {
        s.abs@
    }

    open spec fn c_abs(c: Self::Constants) -> <InductiveMultiplicationApplicationSpec as ApplicationSpec>::Constants {
        c
    }

    closed spec fn inv(&self) -> bool {
        &&& self.client_conn == self.abs@.client_conn
        &&& self.addition_conn == self.abs@.addition_conn
        &&& self.next_seq_no == self.seq_no_assgn@.len()
        &&& self.requests@ == self.abs@.requests
        &&& self.seq_no_assgn@ == self.abs@.seq_no_assgn
        &&& self.first_seq_no@ == self.abs@.first_seq_no
    }
    ...
    {
        InductiveMultiplicationApplication { 
            client_conn: c.0,
            addition_conn: c.1,
            next_seq_no: 0,
            requests: HashSet::new(),
            seq_no_assgn: HashMap::new(),
            first_seq_no: HashMap::new(),
            abs: Ghost(InductiveMultiplicationApplicationSpec {
                client_conn: c.0,
                addition_conn: c.1,
                requests: Set::<MultiplicationRequest>::empty(), 
                replies: Set::<MultiplicationReply>::empty(),
                seq_no_assgn: Map::<SeqNo, MultiplicationRequest>::empty(), 
                first_seq_no: Map::<MultiplicationRequest, SeqNo>::empty(), 
                intermediate_results: Map::<MultiplicationRequest, Seq<AdditionReply>>::empty()
            })
        }
    }
    ...
    {      
        if (recv.0.eq(&self.client_conn)) {
            let parsed_recv = MultiplicationRequest::parse(&recv.1);
            if (parsed_recv.is_some() && !self.requests.contains(&parsed_recv.unwrap())) {
                let parsed_req = parsed_recv.unwrap();

                let parsed_add_request = AdditionRequest { seq_no: self.next_seq_no, x: 0, y: parsed_req.y };
                let add_request = AdditionRequest::marshall(&parsed_add_request);
            }
        } else if (recv.0.eq(&self.addition_conn)) {
            let parsed_recv = AdditionReply::parse(&recv.1);
